...
import { getChallengePdf } from '~/shared/qesClaimApi';
import View from './requestQesClaimView';
...
export default compose<PropsInner, PropsOuter>(
    ...
    withHandlers<PropsOuter & PropsOwn & StateSetter<State> & RouterProps, Handlers>({
        handleRequest: (props) => async (values) => {
            ...
            try {
                const blob = await getChallengePdf(props.id, password);
                saveAs(blob, 'declaration.pdf');
